...
import akka.actor.{ActorSystem, ActorRef, Actor, Props, PoisonPill}
...
object PingPong {
  ...
  class Pinger extends Actor {
    ...
    override def receive: Receive = {
      case Pong => println(s"${self.path.name}: message received $countDown")
  ...
  class Ponger(pinger: ActorRef) extends Actor {
    override def receive: Receive = {
      case Ping => println(s"${self.path.name}: message sent")
        pinger ! Pong
    }
  }
